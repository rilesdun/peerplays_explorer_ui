      ...
      .then((data) => {
        ...
        accountDiv.innerHTML = `
          <div class="bg-black/60 to-white/5 rounded-lg p-2">
            ...
            <p class="text-sm">Account ID: ${account.id}</p>
            <p class="text-sm">Memo Key: ${account.options.memo_key}</p>
            <p class="text-sm">Active Key: ${account.active.key_auths[0][0]}</p>
            <p class="text-sm">Owner Key: ${account.owner.key_auths[0][0]}</p>
          </div>
        `;
      })
